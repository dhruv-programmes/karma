...
```mermaid
graph TD
    User([📱 Consumer Device]) -->|Fluid Interactions| Mobile[apps/mobile: React Native & Expo 57]
    Mobile -->|Fast Navigation| Dock[Custom Glass Floating Dock: 6 Core Paths]
    
    Mobile -->|FastAPI Client| API[apps/api: Python 3.12 & FastAPI]
    Mobile -->|Document Ingestion| AI[apps/ai: Next.js & Vercel AI SDK]
    
    AI -->|Multi-Modal Vision| Gemini[Google Gemini 1.5/2.0 Vision Engine]
    Gemini -->|Extracted Invoices & Bills| API
    
    API -->|Lifecycle Matrices| DB[(Circularity & Carbon Lifecycle DB)]
    API -->|Score Algorithm| KCS[KCS Engine: 480-820 Index & Verification]
    API -->|Redemption Engine| PartnerHub[Govt Subsidy & Brand Offers Service]
```
...
* **AI Extraction Microservice (`apps/ai`)**: Next.js 15, Vercel AI SDK, Google Gemini Vision API for zero-latency document parsing.
...
cd apps/ai
pnpm install
cp .env.example .env.local  # Populate GOOGLE_GENERATIVE_AI_API_KEY
pnpm dev
```
